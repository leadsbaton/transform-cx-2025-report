...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (downloadForm) {
      downloadForm.addEventListener('submit', function(event) {
        ...
        if (!firstName || !lastName || !companyName || !jobTitle || !companyEmail || !phoneNumber) {
    ...
    buttons.forEach(button => {
      button.addEventListener('click', function(e) {
        // If the button is not a submit button in a form
        if (!this.type || this.type !== 'submit') {
          e.preventDefault();
          
          // In a real application, you might want to scroll to a specific section
          // or trigger a download
          if (this.textContent.trim().toLowerCase().includes('download')) {
